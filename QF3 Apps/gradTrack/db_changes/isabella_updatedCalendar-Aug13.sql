...
SET @user = ?;               
SET @block= ?;      
SELECT rppWeek, ''newVisit'' as form,  [Sunday] AS Sun, [Monday] as Mon, [Tuesday] AS Tue, [Wednesday] as Wed, [Thursday] as Thu, 
...
WHERE addedBy=@user and blockKey=@block and deleteflag<>1) ps           PIVOT        ( COUNT (BlockKey)        
FOR ValDayName IN     ([Sunday], [Monday], [Tuesday], [Wednesday], [Thursday], [Friday], [Saturday])) AS pv      
UNION      (SELECT rppWeek, ''newVisit'' as form, ''0'' as Sun,''0'' as Mon, ''0'' as Tue, ''0'' as Wed, ''0'' as Thu, ''0'' as Fri, ''0'' as Sat       
FROM LKUP_date WHERE block = @block and (NOT (rppWeek IS NULL)) group by rppWeek     EXCEPT      
SELECT DISTINCT  LKUP_date.rppWeek, ''newVisit'' as form, ''0'' as Sun,''0'' as Mon, ''0'' as Tue, ''0'' as Wed, ''0'' as Thu, ''0'' as Fri, ''0'' as Sat     
