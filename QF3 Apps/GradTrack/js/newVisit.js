define(['dom/form/form','server/executeQuery'],function(){ 
	...
	$('#preOneSemester, #preTwoSemester, #preThreeSemester').on('click',function(){
		var text = "",
			totalNote = "",
			noteDom = $('#noteInput');
		if (text.indexOf('|') !== -1)
		...
		var note1 = $('#preOneSemester').is(':checked');
		var note2 = $('#preTwoSemester').is(':checked');
		var note3 = $('#preThreeSemester').is(':checked');
		
		if(note1)
			totalNote +="preOneSemester";
		if(note2)
			totalNote+="preTwoSemester";
		if(note3)
			totalNote+="preThreeSemester";
		if (totalNote == "")	
			noteDom.val(text);
		else
